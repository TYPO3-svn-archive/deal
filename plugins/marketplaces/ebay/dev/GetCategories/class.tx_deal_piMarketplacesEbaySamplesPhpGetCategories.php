...
class tx_deal_piMarketplacesEbaySamplesPhpGetCategories extends tslib_pibase
{
  ...
  public $scriptRelPath = 'plugins/marketplaces/ebay/dev/GetCategories/class.tx_deal_piMarketplacesEbaySamplesPhpGetCategories.php';
  public $extKey = 'deal';
...
// Check to see if the call was successful, else print an error
    if ($resp->Ack == "Success")
    {
      $manual = $this->pi_getLL('promptManualSuccess');
      $results = '<tr>'
              . '<th>uid</th>'
              . '<th>pid</th>'
              . '<th>uid_parent</th>'
      ...
      foreach ($resp->CategoryArray->Category as $category)
      {
        ...
        $pid = (int) $this->conf['constanteditor.']['pid']; // Page ID
    ...
    if ($resp->Ack != "Success")
    {
      $manual = $this->pi_getLL('promptManualError');
      $results = '<h2 style="color:red;">
    ...
    $content = '
      <h1>
        ' . $this->pi_getLL('promptSuccessHeader') . '
      </h1>
      ' . $manual . '
  ...
  private function constructPostCallAndGetResponse($endpoint)
  {
    ...
    $levelLimit = (int) $this->conf['constanteditor.']['levelLimit'];     //
    if ($levelLimit > 0)
    {
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/deal/plugins/marketplaces/ebay/dev/GetCategories/class.tx_deal_piMarketplacesEbaySamplesPhpGetCategories.php'])
{
  include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/deal/plugins/marketplaces/ebay/dev/GetCategories/class.tx_deal_piMarketplacesEbaySamplesPhpGetCategories.php']);
}
